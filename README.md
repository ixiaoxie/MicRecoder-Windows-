...
2.  **设置热键**: 
    *   在“热键配置”区域，点击输入框。
    *   直接按下键盘上的组合键（默认 `Ctrl` + `1`），软件会自动识别。
    *   配置好“开始/暂停”和“停止”热键后，点击“保存并应用热键”。
3.  **开始录音**: 
    *   按下您设置的开始热键（默认 `Ctrl+2`），或点击界面上的“开始”按钮。
...
**如果你要运行程序：** 直接运行 `mic_recorder.py`。

**如果你发现图标报错：** 先运行 `convert_icon.py` 生成图标文件。

**如果你发现开机自启没反应：** 运行 `verify_startup.py` 查看报错信息，或运行 `debug_env.py` 查看系统环境。
